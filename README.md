...
(run via the WSL/Linux build -- see [MEMORY USE](#memory-use) below for why
that's the recommended path at this scale on Windows).
...
see [MEMORY USE](#memory-use) below). An older positional form,
`<digits> [output_file] [threads]`, also still works, but `-o`/`-t` are
recommended: with the positional form, a thread count typed without an
output file first (e.g. `pi_calculator 500000000 8`) silently becomes the
OUTPUT FILENAME instead, not the thread count -- easy to trip over. `-o`/`-t`
...
way the one described in the [appendix](#appendix-two-questions-answered-in-more-depth)
below was diagnosed. Ordinary runs don't need it.
...
| 100 million | ~1.2 GB |
| 300 million | ~3.5 GB |
| 500 million | ~6 GB (single-threaded; more with multiple threads, since each can hold its own large numbers in memory at once) |
...
requested size. Full evidence and explanation are in the
[appendix](#appendix-two-questions-answered-in-more-depth) at
...
### Option C: WSL (build a Linux binary -- recommended for very large digit counts, per MEMORY USE above)
...
### 2. What was the "Cannot allocate memory" bug at very large digit counts, and how was it actually found?
...
[msys2/MINGW-packages#8781](https://github.com/msys2/MINGW-packages/issues/8781)
